  ...
  constructor({ clientId, clientSecret, assetPin, aesKey, sessionId, privateKey, shareSecret = '', timeout = 3600 }) {
    ...
    Object.keys(endpoints).forEach(key => {
      ...
      if (!['get', 'post'].includes(api.method)) {
        console.warn('Ignore invalid MixinClient endpoint declaration');
        return;
      }

      debug('Generate method for: ', key);
      this[key] = async (param) => {
        let url = api.url;
        let payload = '';

        // TODO: how do we support multiple params here
        if (api.method === 'get' && api.param) {
          url = strformat(api.url, { [api.param.name]: param });
        } else if (api.method === 'post' && api.param) {
          payload = JSON.stringify(param);
        }

        const data = await this.doRequest(api.method.toUpperCase(), url, payload);
        return data;
      };
    });
  }
