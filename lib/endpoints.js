...
const validateAddressId = (addressId, client) => (client.validateUUID(addressId) ? true : new Error('addressId not valid'));
  ...
  createConversation: {
    ...
    payload: {
      format: (args, client) => {
        ...
        const conversationId = args.category === 'CONTACT' ? client.getConversationId(client.clientId, participants[0].user_id) : client.getUUID();
  ...
  sendMessage: {
    ...
    payload: {
      format: (args, client) => ({
        ...
        message_id: client.getUUID(),
        data: new Buffer(args.message).toString('base64'),
      }),
      validate: (args, client) => {
        if (!['PLAIN_TEXT', 'PLAIN_IMAGE'].includes(args.category)) {
          return new Error('category must be valid');
        }
        if (!args.message) {
        ...
        if (!client.validateUUID(args.recipientId)) {
          return new Error('recipientId must not be empty');
        }
